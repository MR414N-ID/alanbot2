...
𝘘𝘜𝘖𝘛𝘌𝘚  : ${quotes}
└─────────────────❒`
sendButLocation(from, `${menu}`, "*_© Alan OFFICIAL_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`store`,buttonText:{displayText:'𝙨𝙩𝙤𝙧𝙚 𝙢𝙚𝙣𝙪'},type:1},{buttonId:`sc`,buttonText:{displayText:'SC'},type:1},{buttonId:`catalog`,buttonText:{displayText:'CATALOG MENU'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
𝘘𝘜𝘖𝘛𝘌𝘚  : ${quotes}`
sendButLocation(from, `${menu}`, "*_© Alan OFFICIAL_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`allmenu`,buttonText:{displayText:'ALL MENU'},type:1},{buttonId:`rules`,buttonText:{displayText:'RULES'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Hai Kak ${pushname}, ${tampilUcapan}
...
sendButLocation(from, `${menu}`, "*_© Alan OFFICIAL_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`menu`,buttonText:{displayText:'BACK TO MENU'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Hai Kak ${pushname}, ${tampilUcapan}
...
ɢɪᴛʜᴜʙ ᴏᴡɴᴇʀ:https://github.com/MR414N-ID
ʸᵒᵘᵗᵘᵇᵉ ᵒʷⁿᵉʳ:https://youtube.com/c/MR.414N`
sendButLocation(from, `${menu}`, "*_© ALAN OFFICIAL_*", {jpegThumbnail:logonya,name:""}, [{buttonId:`menu`,buttonText:{displayText:'SIAP OM'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'verify':
...
"templateMessage": {
						"hydratedTemplate": {
							...
							"hydratedFooterText": `ALAN OFFICIAL`,
							"hydratedButtons": [
								{
									"quickReplyButton": {
...
case 'chiisaihentai':
              ...
              contentText:`FOLLOW IG _ LANOX`,buttons,headerType:4}
...
case 'bj':
              ...
              imageMsg = (await kurr.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'✗✗✗ ALAN', imageMessage: imageMsg,
              contentText:`Subrek yt : ALAN`,buttons,headerType:4}
              prep = await kurr.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              kurr.relayWAMessage(prep)
...
case 'blackpink':
                ...
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    ...
                    getBuffer(`https://api.lolhuman.xyz/api/textprome/${command}?apikey=KurrXd&text=${ini_txt}`).then((gambar) => {
                        kurr.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `FOLLOW IG _ LANOX`, quoted : ftrol})
                    })
                    break
                case 'pornhub':
                case 'glitch':
                ...
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    ...
                    getBuffer(`https://api.lolhuman.xyz/api/textprome2/${command}?apikey=KurrXd&text1=${txt1}&text2=${txt2}`).then((gambar) => {
                        kurr.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Subrek yt : ALAN`, quoted : ftrol})
                    })
                    break
case 'neko':
       case 'kanna':
...
kurr.sendMessage(from, {url: `${pint[Math.floor(Math.random() * pint.length)]}`}, image, {quoted: mek, thumbnail: Buffer.alloc(0), caption: "FOLLOW IG _ LANOX"})
break
case 'isbaileys': 
case 'bail': 
case 'baileys':
...
Giliran = @${tty.player1.split('@')[0]}`
 kurr.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     kurr.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer ${NamaBot}",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./logonya.jpeg'),sourceUrl:"https://wa.me/p/15512131770/6282292838634"}}})
	}
    }
    }
    }
